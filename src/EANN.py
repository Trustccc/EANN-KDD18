...
class ReverseLayerF(Function):
    ...
    def forward(self, x):
        ...
        # self.save_for_backward(self.lambd)
        return x.view_as(x)
...
def grad_reverse(x):
    return ReverseLayerF.apply(x)  # ReverseLayerF()(x)
...
def main(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (train_data, train_labels, event_labels) in enumerate(train_loader):
            ...
            class_cost_vector.append(class_loss.item())  # class_loss.data[0]
            domain_cost_vector.append(domain_loss.item())  # domain_loss.data[0]
            cost_vector.append(loss.item())  # loss.data[0]
        ...
        for i, (validate_data, validate_labels, event_labels) in enumerate(validate_loader):
            ...
            vali_cost_vector.append(vali_loss.item())  # vali_loss.data[0]
            # validate_accuracy = (validate_labels == validate_argmax.squeeze()).float().mean()
            validate_acc_vector_temp.append(validate_accuracy.item())  # validate_accuracy.data[0]
